	...
	<view class="myTab">
		...
		<view v-show="active === 0">
			<view class="tab-ocean" v-for="f in freightArr" :key="f.F_Id">
				<view class="tabpub-title">{{f.F_ExpenseCodeName}}</view>
			...
			<view class="tab-ocean">
				...
				<view class="oceanfee-item" v-for="pol in polFee" :key="pol.F_Id">
					<view class="ocean-inner">
						<text class="oceanfee-name">{{pol.F_ExpenseCodeName}}</text>
			...
			<view class="tab-ocean">
				...
				<view class="oceanfee-item" v-for="pod in podFee" :key="pod.F_id">
					...
					<view class="oceanfee-mark" v-if="pod.F_IntervalMinPrice || pod.F_IntervalMaxPrice">
						<text v-if="pod.F_IntervalMaxPrice && pod.F_IntervalMinPrice">区间：{{pod.F_IntervalMinPrice}}＜{{pod.F_IntervalUnit}}≤{{pod.F_IntervalMaxPrice}}</text>
						<text v-else-if="pod.F_IntervalMaxPrice">区间：{{pod.F_IntervalUnit}}≤{{pod.F_IntervalMaxPrice}}</text>
						<text v-else-if="pod.F_IntervalMinPrice">区间：{{pod.F_IntervalUnit}}＞{{pod.F_IntervalMinPrice}}</text>
					</view>
							...
							<view class="custom-item">
								...
								>
									<view style="display: flex; align-items: center;">
										...
										<view class="iconfont icon-arrow-down" style="color: #ccc; margin-left: 10px"></view>
									</view>
								</picker>
							</view>
							<view class="custom-item" style="width: 78%;" v-if="indexLand==1">
...
<script>
	...
	export default {
		name: "my-tabbar",
		props: ['lclInfo', 'polFee', 'podFee', 'freightArr', 'applicationLocal', 'lclEfw'],		
		data() {
			return {
				areaArray: [],
				areaCode: [],
		...
		methods: {
			...
			addComputedHandler() {
				...
				// this.packageInfo.number = ''
				// this.packageInfo.weight = ''
				// this.packageInfo.volume = ''
			},
			...
			async getComputedFeeData() {
				...
				const {data: res} = await uni.$http.post('Exportlcl/CalQuotation', {
					...
					Efw_Id: this._props.lclEfw? this._props.lclEfw : '',
