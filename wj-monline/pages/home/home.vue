<template>
	<view class="home">
		<view class="maskPub" v-if="isMaskPub">
			<view class="maskPub-box">
		...
		<view class="home-nav">
			...
			<view class="nav-item" @click="goToExportSearch">
				<view 
					class="iconfont icon-folder-fill"
					style="color: #32a6fd"
				>
				</view>
				<text class="nav-title">空运出口</text>
			</view>
		</view>
		<!-- bussiness type -->
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			setTimeout(() => {
				this.isMaskPub = false
				uni.showTabBar()
			}, 2600) 
			
			// 登录
			this.scene = option.scene? option.scene : ''
			uni.login({
				provider: 'weixin',
		...
		onShow() {
			this.imgUrl = 'https://wxa.worldjaguar.com/webview/images/wj_monline/home_banner.jpg'
		...
		methods: {
			...
			goToExportSearch() {
				uni.navigateTo({
					url:'/pages/subPack/airsearch/airsearch'
				})
			},
