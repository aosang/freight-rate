<template>
  <div class="MyOrder">
    ...
    <el-card class="order-card">
      <el-tag style="margin-right: 20px">海出拼箱</el-tag>
      <el-date-picker 
        v-model="startTime" 
        type="date" 
        placeholder="选择开始时间" 
    ...
    <el-card class="order-card">
      ...
      <el-table :stripe="true" fit :data="savePriceData" border class="order-table"
        ...
        <el-table-column min-width="150" label="货量">
          <template slot-scope="scope">
            {{ scope.row.F_PCSQuantity }}{{ scope.row.F_PCSUnit === '1' ? 'CTN(S)' : 'PLT(S)' }}/{{
              scope.row.F_GrossWeight
              + 'KGS' }}/{{ scope.row.F_Volume + 'CBM' }}
          </template>
        </el-table-column>
    ...
    <!-- 报价单详情 -->
    <el-dialog title="报价单详情" width="75%" :visible.sync="dialogVisible">
      ...
      <div class="model-info-detail">
        <div class="detail-group">
          <div class="detail-item">
            <span>起运港码头</span>
            <p>{{ lclInfo.FromQuayName? lclInfo.FromQuayName : '' }}{{ lclInfo.FromQuayEng? lclInfo.FromQuayEng : ''  }}</p>
          </div>
          <div class="detail-item">
            <span>目的港码头</span>
          ...
          <div class="detail-item">
            <span>截单日期</span>
            <p>{{ lclInfo.F_CutoffTime }}</p>
          </div>
          <div class="detail-item">
            <span>截货日期</span>
            <p>{{ lclInfo.F_CutoffGoodsTime === null ? '--' : lclInfo.F_CutoffGoodsTime }}</p>
          </div>
          <div class="detail-item">
            <span>截仓时间</span>
        ...
        <div class="detail-group">
          <div class="detail-item" style="width: 100%;">
            <div v-if="lclInfo.Efw_WarehouseArr || lclInfo.Efw_WarehouseArr == []">
              <span>接货仓库</span>
              <div v-for="s in lclInfo.Efw_WarehouseArr" :key="s.F_Id" style="display: flex;">
                <p style="margin-bottom: 5px;">
                  {{ s.F_WarehouseName? s.F_WarehouseName : '-' }}
                </p>
                <el-popover placement="right" width="200" trigger="click">
                  <span 
                    class="el-icon-user-solid" 
                    @click="visible = !visible, getStoreDataInfo(s.F_Id)" slot="reference">
                  </span>
