        ...
        <div class="Search-item">
          ...
          <div class="Lcl-input">
            ...
            <el-select 
              ...
              remote
              clearable
              placeholder="起运港国家/地区"
              @focus="getPodCountryData"
              @change="handlerChangeIcl"
            >
              <el-option 
                v-for="s in podCountryArr" 
                :key="s.F_Id" 
                :value="s.F_Id" 
                :label="s.F_CountryEnglishName + ' ' + s.F_CountryName"
              >
              </el-option>
            </el-select>
          </div>
...
import DataIList from '@/components/DataList/DataIList'
...
export default {
  ...
  created() {
    ...
    this.ilclCity = this.$route.params.ilclCity
    this.isearchPolText = this.$route.params.isearchPolText
    this.isearchPodText = this.$route.params.isearchPodText

    this.getPolListData(this.isearchPolText, 2)
    this.getPodListData(this.isearchPodText, 2)
  ...
  mounted() {
    ...
    //调用查询列表
    this.getILclListData(this.currentPage)
  },

  methods: {
    handleCurrentChange(newPage) {
      this.getILclListData(newPage)
    },
    ...
    remotePol(query) {
      this.getPolListData(query, 2)
    },

    remotePod(query) {
      this.getPodListData(query, 2)
    },
    ...
    async getTransPortStoreData(num) {
      const {data: res} = await this.$http.post('data/getImLclPortStore', {
        Type: num,
        From: this.startCity,
        To: this.endCity,
        CountryId: this.ilclCity
