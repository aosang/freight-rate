				...
				<view class="oceanfee-item" v-for="pol in polFee" :key="pol.F_id">
					<view class="ocean-inner">
						<text class="oceanfee-name">
							{{applicationLocal =='zh-Hans'? pol.F_ExpenseCodeName : pol.F_ExpenseCodeEng}}
						</text>
				...
				<view class="oceanfee-item" v-for="pod in podFee" :key="pod.F_Id">
					<view class="ocean-inner">
						...
						<text style="margin-left: 30rpx">
							{{$t('Imlcl.type')}}&nbsp;{{applicationLocal == "zh-Hans"? pod.ExpenseTypeName : pod.ExpenseTypeNameEn}}
						</text>
					</view>
					<view class="oceanfee-mark">
			...
			<view class="computed-sea" v-if="isComputed">
				...
				<view class="pub-item" v-for="f in podFeeComputed" :key="f.F_Id">
					...
					<view class="pub-item-type">
						{{$t('Imlcl.type')}}：{{ applicationLocal == 'zh-Hans'? f.ExpenseTypeName? f.ExpenseTypeName : '--' : f.ExpenseTypeNameEn? f.ExpenseTypeNameEn : '--' }}
					</view>
					<view class="pub-result">
						<text v-if="f.F_QuotedPrice">
							{{f.F_QuotedPrice}}{{f.F_Currency}}*{{f.F_Quantity + '(' + f.F_Unit + ')'}} = {{f.F_QuotedAmount}}{{f.F_Currency}}
		...
		methods: {
			...
			getDestTypeTotal(dataArr) {
				...
				if (this._props.applicationLocal == 'zh-Hans') {
					...
					newArr = _.groupBy(dataArr, 'ExpenseTypeNameEn')
				}
				for (let key in newArr) {
					sum = 0
					newArr[key].forEach(item => {
