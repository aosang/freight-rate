...
<script>
	const CHINA_REGIONS = require('./area.json')
	export default {
		props: {
			defaultRegions: {
				type: Array,
		...
		computed: {
			multiArray() {
				return this.pickedArr.map(arr => arr.map(item => item.enname))
			},
			pickedArr() {
				// 进行初始化
			...
			handleDefaultRegion(region) {
				...
				for (let i = 0; i < 3; i++) {
					for (let j = 0; j < children.length; j++) {
						let condition = isCode ? children[j].id == region.slice(0, (i + 1) * 2) : children[j].enname.includes(region[i]);
						if (condition) {
							// 匹配成功进行赋值
