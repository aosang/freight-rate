<template>
	<view class="landsearch">
		<view class="landsearch-inner">
			...
			<view class="landsearch-item">
				<text class="landsearch-item-title">
					<text class="require" style="margin-left: 4rpx"></text>{{$t('ltl.postcode')}}
				</text>
				<view class="landsearch-item-picker">
			...
			<view class="landsearch-item">
				...
				<view class="landsearch-item-picker" @click="showLandPodData('showLeft')" style="position: relative; z-index: 1;">
					<input type="text" v-model="landPodTextSelected" :placeholder="$t('ltl.selectport')">
				</view>
				<view class="landsearch-item-unit"></view>
				<!-- 收货地列表 -->
			...
			<view class="landsearch-item">
				<text class="landsearch-item-title"><text class="require">*</text>QTY:</text>
				<view class="landsearch-item-picker">
					<input placeholder-class="lclinputstyle" type="number" v-model.number="landInfo.number" placeholder="Enter QTY">
				</view>
			</view>
			<view class="landsearch-item">
				<text class="landsearch-item-title"><text class="require">*</text>G.W.:</text>
				<view class="landsearch-item-picker">
					<input placeholder-class="lclinputstyle" type="digit" v-model.number="landInfo.weight" placeholder="Enter G.W.">
				</view>
			</view>
			<view class="landsearch-item">
				<text class="landsearch-item-title"><text class="require">*</text>SIZE:</text>
				<view class="landsearch-item-picker">
					<input placeholder-class="lclinputstyle" type="digit" v-model.number="landInfo.volume" placeholder="Enter SIZE">
				</view>
			</view>
			<view class="landsearch-item" style="display: flex">
				<button type="primary" class="search-btn" @click="goToLandListPage">Query</button>
				<button class="search-reset" @click="clearLandInfo">Reset</button>
			</view>
		</view>
		<text class="landsearch-tips">*Pick up place and postal code must be at least one entry</text>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			regionName() {
				return this.areaArray.map(item => item.enname).join('-')
			}
		},
		...
		methods: {
			...
			goToLandListPage() {
				...
				if(!this.areaCode && !this.zcode) {
					uni.$showMsg('Enter Pick-up Address or ')
				}else if(!this.landPodTextSelected) {
					uni.$showMsg('Enter Port')
				}else if(!number) {
					uni.$showMsg('Enter QTY')
				}else if(!weight) {
					uni.$showMsg('Enter G.W.')
				}else if(!volume) {
					uni.$showMsg('Enter SIZE')
				}else {
					uni.navigateTo({
