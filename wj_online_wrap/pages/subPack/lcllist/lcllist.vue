	...
	<view class="lcllist">
		...
		<view class="lcllist-box">
			...
			<view class="lcllist-item"
				...
				<view class="lcllist-top">
					...
					<view class="info">
						<text class="info-week">{{$t('lcllist.etd')}}{{applicationLocal == 'zh-Hans'? lcl.F_ShiftPeriod : lcl.F_ShiftPeriodEn}}</text>
						<text class="info-company">{{$t('lcllist.carrier')}}{{lcl.F_ShoppingCompany? lcl.F_ShoppingCompany : '--'}}</text>
					</view>
					<view class="info">
						<text class="info-company">
							<text class="info-day">{{$t('lcllist.voyage')}}{{lcl.F_Voyage? lcl.F_Voyage + $t('pub.day') : '--'}}</text>
		...
		<uni-popup ref="expenseFee" type="bottom" background-color="#fff">
			<view class="consignee-box">
				<view 
					class="consignee-box-item" 
					v-for="e in expenseFeeArr" 
					:key="e.id"
					@click="selectexpenseFee(e.id)"
				>
					{{e.name}}
				</view>
			</view>
	...
	export default {
		data() {
			return {
				...
				expenseFeeArr: [{id: 1, name: this.$t('lcllist.normal')}, {id: 2, name: this.$t('lcllist.high')}, {id: 3, name: this.$t('lcllist.low')}],
				consigneeId: '',
				transPortId: '',
				expenseId: ''
			}
		...
		methods: {
			...
			detailsHandler(lclId) {
				...
				uni.navigateTo({
					url: `/pages/subPack/lcldetails/lcldetails?lclId=${lclId}`
				})
			},
