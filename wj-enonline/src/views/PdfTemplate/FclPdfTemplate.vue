<template>
  <div class="pdfTemplateBox">
        ...
        <div class="wj-template-info">
          <div class="wj-template-info-l">
            <div class="info-l-head">
              ...
              <div class="info-l-name">
                <h3>{{userInfo.CompanyName? userInfo.CompanyName : 'World Jaguar'}}</h3>
                <p>{{userInfo.EnCompanyName? userInfo.EnCompanyName : 'World Jaguar Logistics Inc.'}}</p>
              </div>
            </div>
            <div class="info-l-info">
              <p>{{ lclInfo.FromPortEnglish }}{{ '(' + lclInfo.FromPortName + ')' }} -- {{ lclInfo.ToPortEnglish }}{{ '(' + lclInfo.ToPortName + ')' }}</p>
              <p>Carrier:{{ lclInfo.F_ShoppingCompany }}&nbsp;{{lclInfo.F_SpellState === '1'? 'Direct' : 'Transfer'}}&nbsp;&nbsp;{{ lclInfo.F_Voyage? lclInfo.F_Voyage : '-' }}Days</p>
              <p v-if="lclInfo.TransitPortName">Transit:{{ lclInfo.TransitPortEnglish }} {{ lclInfo.TransitPortName }}</p>
              <p>Schedule:{{lclInfo.F_ShiftPeriod}}&nbsp;&nbsp;&nbsp;&nbsp;Cargo information:<span>{{ cargoInfo }}</span></p>
              <p>Valid:{{ lclInfo.F_StartTime }} to {{ lclInfo.F_EndTime }}</p>
              <p v-if="lclInfo.F_BookingRemarks">Booking Remark:{{ lclInfo.F_BookingRemarks }}</p>
              <p v-if="lclInfo.F_WeightLimitRemarks">Weight Limit Remark:{{ lclInfo.F_WeightLimitRemarks }}</p>
            </div>
          </div>
          <div class="wj-template-info-r">
            ...
            <div class="info-contact">
              <p>Contact:{{ userInfo.F_RealName }}</p>
              <p>Mobile:{{ userInfo.F_Mobile? userInfo.F_Mobile : '--' }}</p>
              <p>Email:{{ userInfo.F_Email? userInfo.F_Email : '--' }}</p>
              <p>Time:{{ dateText }}&nbsp;{{ timeText }}</p>
            </div>
          </div>
        </div>
        <el-alert 
          ...
          title="The blue font indicates that the fee is a single ticket fee"
        ...
        <div class="wj-template-table" v-if="freight.length !== 0">
          <div class="wj-template-title">
            ...
            <div style="display: flex">
              ...
              <span v-if="singlePriceFreight">SingleVote:{{singlePriceFreight}}</span>
              <span style="margin: 10px 15px 10px auto; color: #F56C6C">(The cost is for reference only)</span>
            </div>
          ...
          >
            <el-table-column label="No." type="index" width="55" :index="indexMethod"></el-table-column>
            ...
            <el-table-column prop="F_Unit" label="Unit">
              <template slot-scope="scope">
                <span>{{ scope.row.F_Unit }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="F_UnitPrice" label="UnitPrice">
              <template slot-scope="scope">
        ...
        <div class="wj-template-table" v-if="polFee.length !== 0">
          <div class="wj-template-title">
            ...
            <div style="display: flex">
              ...
              <span v-if="singlePricePol">SingleVote{{singlePricePol}}</span>
              <span style="margin: 10px 15px 10px auto; color: #F56C6C">(The cost is for reference only)</span>
            </div>
          </div>
          <el-table
        ...
        <div class="wj-template-table" v-if="podFee.length !== 0">
          <div class="wj-template-title">
            ...
            <div style="display: flex">
              ...
              <span v-if="singlePricePod">SingleVote:{{singlePricePod}}</span>
              <span style="margin: 10px 15px 10px auto; color: #F56C6C">(The cost is for reference only)</span>
            </div>
          </div>
          <el-table
    ...
    <div class="wj-template-btn">
      <div class="wi-template-back" @click="closeWindow">Cancel</div>
      <div class="wi-template-pdf" @click="exportPdf">Export PDF</div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      exportPDFtitle: "Worldjaguar FCL",
      pdfData: '',
      num20gp: '',
      num40gp: '',
      num40hq: '',
