<template>
  <div class="DataModel">
    <el-dialog 
      width="83%" 
      title="Export-LCL Quotation Details" 
      :visible.sync="isShow" 
    ...
    >
      <div class="model-info-city">
        ...
        <div class="city-item">
          <label>
            {{ lclInfo.F_Voyage? lclInfo.F_Voyage : '--'}}days
          </label>
          <i></i>
          <label>
            {{lclInfo.TransitPortEnglish? 'Direct' : lclInfo.TransitPortEnglish}}
          </label>
        </div>
        <div class="city-item">
      ...
      <div class="model-info-detail">
        <div class="detail-group">
          ...
          <div class="detail-item">
            ...
            <p>{{lclInfo.F_CutoffGoodsTimeEn? lclInfo.F_CutoffGoodsTimeEn : '-'}}</p>
          </div>
          <div class="detail-item">
            <span>Valid</span>
        ...
        <div class="detail-mark">
          ...
          <div class="mark-item" v-if="lclInfo.F_EnglishRemarks">
            <span>Remark</span>
            <p>{{ lclInfo.F_EnglishRemarks?  lclInfo.F_EnglishRemarks : '-' }}</p>
          </div>
          <div v-else>
            <div class="mark-item">
              <span>订舱备注</span>
      ...
      <el-tabs v-model="activeName" class="model-tabs">
          ...
          <div class="model-fee">
            ...
            <div class="model-fee-item">
              ...
              <el-table border class="fee-table" :data="freightArr" :header-cell-style="{ background: '#f2f4f6' }">
                ...
                <el-table-column prop="F_Currency" label="Currency">
                </el-table-column>
                <el-table-column label="CBM">
                  <template slot-scope="scope">
                    <span v-if="scope.row.F_CBM !== ''">
                      {{ Number(scope.row.F_CBM < 0) ? '(' + Number(scope.row.F_CBM) * -1 + ')' : scope.row.F_CBM }}
                    </span>
                    <span v-else>case by case</span>
                  </template>
                </el-table-column>
                <el-table-column label="TON">
                  <template slot-scope="scope">
                    <span v-if="scope.row.F_TON !== ''">
                      {{ Number(scope.row.F_TON < 0) ? '(' +  Number(scope.row.F_TON) * -1 + ')' : scope.row.F_TON }}
                    </span>
                    <span v-else>case by case</span>
                  </template>
                </el-table-column>
                <el-table-column prop="F_MIN" label="MIN">
                </el-table-column>
                <el-table-column prop="F_RT" label="RT Scope">
                </el-table-column>
            ...
            <div class="model-fee-item">
              ...
              <el-table border class="fee-table" :data="polFee" :header-cell-style="{ background: '#f2f4f6' }">
                ...
                <el-table-column prop="F_ExpensesDescription" label="Remark"></el-table-column>
              </el-table>
              <div style="margin-top: 10px" v-if="textareaFreight">
                <span style="font-size: 14px; margin-bottom: 10px;">Remark：</span>
                <div class="pubFeeRemark">{{ textareaFreight }}</div> 
            ...
            <div class="model-fee-item">
              ...
              >
                ...
                <el-table-column label="Vol.ratio">
                  <template slot-scope="scope">
        ...
        <el-tab-pane label="Edit Quotation" name="2">
          <div class="model-create-item">
            ...
            <p>Vol.ratio:{{this.lclInfo.F_VolumeRatio === '' || Number(this.lclInfo.F_VolumeRatio) === 0? '--' : '1:' + Number(this.lclInfo.F_VolumeRatio)}}</p>
            <p v-if="this.lclInfo.F_RTLowerLimit == '' && this.lclInfo.F_RTUpperLimit == ''">RT Scope:--</p>
            <p v-else-if="this.lclInfo.F_RTLowerLimit !== '' && this.lclInfo.F_RTUpperLimit == ''">RT Scope:{{this.lclInfo.F_RTLowerLimit}}＜RT</p>
            <p v-else-if="this.lclInfo.F_RTLowerLimit == '' && this.lclInfo.F_RTUpperLimit !== ''">RT Scope:RT≤{{this.lclInfo.F_RTUpperLimit}}</p>
            <p v-else-if="this.lclInfo.F_RTLowerLimit !== '' && this.lclInfo.F_RTUpperLimit !== ''">RT Scope:{{this.lclInfo.F_RTLowerLimit}}＜RT≤{{this.lclInfo.F_RTUpperLimit}}</p>
            <p v-else>RT Scope:--</p>
          </div>
          ...
          <div class="model-create-table">
            <div class="table-fee-item" v-if="isComputed">
              ...
              <div class="order-table">
                <el-table 
                  style="width: 100%" 
                  border 
                  :header-cell-style="{ background: '#f2f4f6', fontWeight: '500'}"
                    ...
                    <li>
                      ...
                      >
                        <el-option
                          ...
                          :value="c.F_ItemValue"
                          :label="c.F_ItemValue">
                          <span style="float: left">{{ c.F_ItemValue }}</span>
            ...
            <div class="table-fee-item" v-if="isComputed">
              <div class="fee-info">
                ...
                <span>Convert(USD):{{ converTotalPol? converTotalPol : '--' }}</span>
              ...
              <div class="order-table">
                ...
                <div class="order-edit">
                  <ul>
                    <li>
                      <el-input v-model="addPolFee.F_ExpenseCode" style="width: 180px" placeholder="Charge Name"></el-input>
                    </li>
                    <li>
                      <el-select v-model="addPolFee.F_Currency" placeholder="CCY">
                        <el-option
                          v-for="c in currencyArr"
                          :key="c.F_ItemDetailId"
                          :value="c.F_ItemValue"
            ...
            <div class="table-fee-item" v-if="isComputed">
              ...
              <div class="order-table">
                ...
                >
                  ...
                  <el-table-column prop="F_QuotedAmount" label="Quoted Amount"></el-table-column>
                  <el-table-column prop="F_Balance" label="Balance"></el-table-column>
                  <el-table-column width="280" prop="F_Remark" label="Remark">
                    <template slot-scope="scope">
          ...
          <div class="order-total">Total(USD):{{exportLclUsdTotal}}</div>
          <div class="order-btn">
  ...
  methods: {
    ...
    async computedOecanFee() {
      if (this.quotation.pcs === '') {
        ...
        this.$message({
          message: 'Please enter the package',
          type: 'error',
          duration: 2000
        })
      } else {
      ...
      try {
        if (res.code === 200) {
          ...
          if(this.polFeeComputed.length !== 0) {
            ...
            this.converTotalPol = this.getComputedTotal(this.polFeeComputed)[1]
            this.diffPolTotal = this.getBalanceComputed(this.polFeeComputed)
          }else {
            this.polTotal = '--'
            this.converTotalPol = 0
            this.diffPolTotal = '--'
          }
          ...
          this.getPdfParams(this.lclInfo, this.polFeeComputed, this.podFeeComputed, this.freightComputed, this.quotation.pcs,this.quotation.kgs, this.quotation.cbm, this.selectType, this.freightTotal, this.polTotal, this.podTotal, this.textareaFreight, this.textareaPol,this.textareaPod, this.convertTotal, this.type, this.polMiniCharge)
        } else if (res.code === 0) {
          this.$message({
            message: res.info,
            type: 'error',
    ...
    // 传递导出PDF参数
    getPdfParams(lclInfo, polFee, podFee, freight, pcs, kgs, cbm, freightTotal, polTotal, podTotal, textareaFreight, textareaPol, textareaPod, convertTotal, freightTotalUsd, polTotalUsd, totalUsd, type, miniCharge) {
      let pdfData = { lclInfo, polFee, podFee, freight, pcs, kgs, cbm, freightTotal, polTotal, podTotal, textareaFreight, textareaPol, textareaPod, convertTotal, freightTotalUsd, polTotalUsd, totalUsd, type, miniCharge }
    ...
    addFeeList(type) {
      if(type == 1) {
        ...
        if(!F_ExpenseCode.trim()) {
          this.$message({
            message: 'Please fill in the correct shipping fee',
            type: 'error',
            duration: 2000
          })
        }else if(!F_Currency) {
          this.$message({
            message: 'Please select the correct currency',
            type: 'error',
            duration: 2000
          })
        }else if(!F_UnitPrice) {
    ...
    saveFeeData(type, row ,index) {
      if(row.F_QuotedPrice === '') {
        ...
        if(type == 1) {
          this.freightTotal = this.getComputedTotal(this.freightComputed)[0]
          this.diffFreightTotal = this.getBalanceComputed(this.freightComputed)
          this.getPdfParams(this.lclInfo, this.polFeeComputed, this.podFeeComputed, this.freightComputed, this.quotation.pcs, this.quotation.kgs, this.quotation.cbm, this.selectType, this.freightTotal, this.polTotal, this.podTotal, this.textareaFreight, this.textareaPol,this.textareaPod)
        }else if(type == 2) {
          this.polTotal = this.getComputedTotal(this.polFeeComputed)[0]
          this.diffPolTotal = this.getBalanceComputed(this.polFeeComputed)
          this.getPdfParams(this.lclInfo, this.polFeeComputed, this.podFeeComputed, this.freightComputed, this.quotation.pcs, this.quotation.kgs, this.quotation.cbm, this.selectType, this.freightTotal, this.polTotal, this.podTotal, this.textareaFreight, this.textareaPol,this.textareaPod)
