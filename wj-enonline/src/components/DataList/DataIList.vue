            ...
            <td>
              <div class="lcl-pay">
                <el-popover placement="left" width="1000" title="ORIGIN">
                  <a slot="reference" @click="getPolFeesData(d.F_Id)">
                    ORIGIN
                  </a>
                  <el-table
                    border 
                  ...
                  >
                    ...
                    <el-table-column label="Min">
                      <template slot-scope="scope">
                        {{ scope.row.F_MiniCharge? scope.row.F_MiniCharge : '' }}{{ scope.row.F_MaxUnit && scope.row.F_MiniCharge?  '/' + scope.row.F_MaxUnit : '' }}
                      </template>
                    </el-table-column>
                    <el-table-column width="100" label="Max">
                      <template slot-scope="scope">
                        {{ scope.row.F_MaxCharge? scope.row.F_MaxCharge : '' }}{{ scope.row.F_MaxUnit && scope.row.F_MaxCharge? '/' + scope.row.F_MaxUnit : '' }}
                      </template>
                    </el-table-column>
                ...
                <el-popover placement="left" width="1000" title="DEST">
                  <a slot="reference" @click="getPodFeesData(d.F_Id)">
                    DEST
                  </a>
                  <el-table
                    border 
                    ...
                    max-height="400"
                  >
                    <el-table-column prop="ExpenseTypeNameEn" label="Type"></el-table-column>
                    <el-table-column prop="F_ExpenseCodeEng" label="Charge Name"></el-table-column>
                    <el-table-column prop="F_Currency" label="Currency"></el-table-column>
                    <el-table-column prop="F_UnitPrice" label="Rate"></el-table-column>
                    <el-table-column prop="F_Unit" label="Unit"></el-table-column>
                    <el-table-column width="100" prop="F_MiniQuantity" label="Min Quantity"></el-table-column>
                    <el-table-column width="100" prop="F_MiniAmount" label="Min Rate"></el-table-column>
                    <el-table-column label="Remark">
                      <template slot-scope="scope">
                        <el-tooltip 
                        ...
                        >
                          <div slot="content" style="max-width: 600px; line-height: 20px">
                            {{scope.row.F_ExpensesDescription}}
                          </div>
                          <el-button size="mini">Details</el-button>
                        </el-tooltip>
                        <div v-else>-</div>
...
import DataIModel from './DataModel/DataIModel'
export default {
  name: 'DataList',
  data() {
    return {
      ...
      mergeArr: ['F_Id', 'F_Description'],// 表格中的列名
      isLoading: true
    }
  },
  ...
  methods: {
    ...
    async getDetailsInfo(id) {
      const { data: res } = await this.$http.post('Imlcl/lclInfo', {
        LclId: id
      })
      try {
        if (res.code !== 200) {
    ...
    async getPolFeesData(id) {
      const { data: res } = await this.$http.post('Imlcl/lclFee', {
        Type: 1,
        LclId: id
      })
      try {
        if (res.code !== 200) {
          ...
          this.getSpanArr(this.originArr)
